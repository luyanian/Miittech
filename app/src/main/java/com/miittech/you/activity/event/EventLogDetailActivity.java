        ...
        switch (view.getId()) {
            case R.id.img_navagation:
                if(AppUtils.isInstallApp("com.baidu.BaiduMap")){
                    ...
                    i1.setData(Uri.parse("baidumap://map/geocoder?location="+eventlistBean.getLocinfo().getLat()+","+eventlistBean.getLocinfo().getLng()));
                    startActivity(i1);
                }else{
                    Intent l2 = new Intent(Intent.ACTION_VIEW);
                    l2.setData(Uri.parse("http://api.map.baidu.com/geocoder?location="+eventlistBean.getLocinfo().getLat()+","+eventlistBean.getLocinfo().getLng()
